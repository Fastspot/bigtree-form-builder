...
<section>
	...
	<fieldset>
		...
		<div id="form_builder_max_entries"<?php if (!$form["limit_entries"]) { ?> style="display: none;"<?php } ?>>
			<label for="form_builder_max_entries_field">Maximum Number of Entries</label>
			<input id="form_builder_max_entries_field" type="text" name="max_entries" value="<?=$form["max_entries"]?>" />
		</div>
	</fieldset>
	<?php if (!empty($settings["accept_payments"])) { ?>
	<fieldset>
		...
		<div id="form_builder_early_bird" class="form_builder_triplets"<?php if (!$form["paid"]) { ?> style="display: none;"<?php } ?>>
			<input type="checkbox" name="early_bird" <?php if ($form["early_bird_date"]) { ?> checked="checked"<?php } ?> />
			<label class="for_checkbox">Early Bird Pricing</label>
		</div>
	</fieldset>
	<div id="form_builder_paid_extras"<?php if (!$form["paid"]) { ?> style="display: none;"<?php } ?>>
		<div class="left" id="form_builder_base_price"<?php if (!$form["paid"]) { ?> style="display: none;"<?php } ?>>
			<fieldset>
		...
		<div class="right" id="form_builder_early_base_price"<?php if (!$form["early_bird_date"]) { ?> style="display: none;"<?php } ?>>
			<fieldset>
		...
		<div class="left" id="form_builder_early_bird_date"<?php if (!$form["early_bird_date"]) { ?> style="display: none;"<?php } ?>>
			<label for="form_builder_early_bird_date_field">Early Bird Cut-off Date <small>(i.e. February 22, 2011 @ 5:30pm)</small></label>
			<input id="form_builder_early_bird_date_field" type="text" name="early_bird_date" value="<?php if ($form["early_bird_date"]) { echo date("F j, Y @ g:ia", strtotime($form["early_bird_date"])); } ?>" />
		</div>
	</div>
	<?php } ?>
