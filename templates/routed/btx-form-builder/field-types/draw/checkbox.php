...
<fieldset>
	...
	<div class="form_builder_checkbox">
		<input type="checkbox" id="form_builder_field_<?=$count?>" name="<?=$field_name?><?php if (count($field_data["list"]) > 1) { ?>[]<?php } ?>" value="<?=$value?>"<?php if ((is_array($default) && in_array($value,$default)) || ($default === false && $item["selected"])) { ?> checked="checked"<?php } ?> data-price="<?=$item["price"]?>" /> 
